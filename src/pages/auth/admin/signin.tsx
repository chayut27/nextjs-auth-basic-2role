...
const SignIn = (props: Props) => {
  ...
  const onSubmit: SubmitHandler<Inputs> = async data => {
    let callbackUrl = window.location.origin+"/admin/dashboard"
    if (query.callbackUrl) {
      callbackUrl = query.callbackUrl as string;
    }
    await signIn("admin-login", { username: data.username, password: data.password, callbackUrl: callbackUrl })
            ...
            <div className="card">
              <div className="card-body">
                <form onSubmit={handleSubmit(onSubmit)}>
                  <div className='mb-3'>
                    <input type="text" className="form-control" placeholder='Username' defaultValue="" {...register("username", { required: true })} />
                    {errors.username && <span className='small text-danger'>This field is required</span>}
                  </div>
                  <div className='mb-3'>
